...
function getInitialJson (cb) {
  var id = $('meta[name="cmodelID"]').attr('content'),
    ...
    // api = "https://documenter.getpostman.com/api/collection/1666902/6mz5cLC"
    api = "postmanJson.json"
  ...
  if (id || (ownerId && publishedId)) {
    $.getJSON(api)
      .done(function (data, status, jqxhr) {
        console.log('getInitialJson.data', data);
        console.log('getInitialJson.done', arguments);
        return cb(data);
      })
      .fail(function (jqxhr, status, statusText) {
        console.log('getInitialJson.jqxhr', jqxhr);
        console.log('getInitialJson.status', status);
